...
class InferenceNode(Node):
    ...
    def __init__(self):
        ...
        # Model checkpoint path - configurable via FRUIT_CLASSIFIER_MODEL_PATH environment variable
        # For deployment, set this to the absolute path where the model is stored on the robot
        model_path = os.getenv(
            "FRUIT_CLASSIFIER_MODEL_PATH",
            "/home/ubuntu/models/efficientnet_b0_group_best_no_aug.pth"  # Default deployment path
        )
        
        if not os.path.exists(model_path):
            self.get_logger().warn(
                f"Model checkpoint not found at {model_path}. "
                "Please set FRUIT_CLASSIFIER_MODEL_PATH environment variable to the correct path."
            )
